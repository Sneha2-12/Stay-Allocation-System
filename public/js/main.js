...
const prefBedding = document.getElementById('pref-bedding');
...
function loginUserSession(user) {
  ...
  if (user.role === 'manager') {
    ...
    // Add "Add Room" button in header actions
...
function setupEventListeners() {
  ...
  preferencesForm.addEventListener('submit', async (e) => {
    ...
    const extraBedding = prefBedding.checked;
    
    // Gather add-ons
    const addOns = [];
    if (prefBreakfast.checked) addOns.push('breakfast');
    ...
    try {
      ...
      if (data.success) {
        ...
        // Switch to Stays view, which will load recommended rooms
        switchView('rooms-view');
      }
    } catch (err) {
      alert('Error saving your stay configurations.');
    }
  });
    ...
    try {
      const res = await fetch('/api/allocations', {
        ...
        body: JSON.stringify({ 
          ...
          extraBedding: prefBedding.checked, 
          addOns, 
          nights, 
          notes 
        })
...
function setupStayPlannerInteractions() {
  ...
  [prefGuests, prefNights, prefBedding, prefBreakfast, prefShuttle, prefLateCheckout].forEach(input => {
...
function updateLiveEstimator() {
  ...
  // Bedding
  if (prefBedding.checked) {
    const cost = 30 * nights;
    estBeddingPrice.textContent = `$${cost.toFixed(2)}`;
  ...
  // Breakfast
  if (prefBreakfast.checked) {
    const cost = 20 * guests * nights;
    estBreakfastPrice.textContent = `$${cost.toFixed(2)}`;
      ...
      if (data.success) {
        ...
        if (pending.length === 0) {
          ...
          pendingAllocationsTableBody.innerHTML = pending.map(a => {
            ...
            const addOnsList = [];
            if (a.extraBedding) addOnsList.push('Extra Bed');
            if (a.addOns.includes('breakfast')) addOnsList.push('Breakfast');
            if (a.addOns.includes('shuttle')) addOnsList.push('Shuttle');
            if (a.addOns.includes('lateCheckout')) addOnsList.push('Late Checkout');
            ...
            return `
              <tr>
                ...
                <td style="font-size:0.8rem; color:var(--text-secondary);">${addOnsStr}</td>
        ...
        if (activeAlloc && activeAlloc.status === 'pending') {
          ...
          mainContentHtml = `
            <div class="chart-container" style="padding: 2.5rem;">
              <h3 style="margin-bottom: 0.5rem;">Booking Reservation Pending</h3>
          ...
          const addOnsList = [];
          if (approvedAlloc.extraBedding) addOnsList.push('Extra Bedding');
          if (approvedAlloc.addOns.includes('breakfast')) addOnsList.push('Daily Breakfast');
          if (approvedAlloc.addOns.includes('shuttle')) addOnsList.push('Airport Shuttle');
          if (approvedAlloc.addOns.includes('lateCheckout')) addOnsList.push('Late Checkout');
          const addOnsStr = addOnsList.length > 0 ? addOnsList.join(', ') : 'None';
            ...
            <div class="chart-container" style="padding: 2.5rem; display: grid; grid-template-columns: 2fr 1fr; gap: 2rem;">
              <div>
                ...
                <div style="margin-top: 1.5rem; display: flex; flex-direction: column; gap: 0.75rem;">
                  ...
                  <p><strong>Selected Add-ons:</strong> ${addOnsStr}</p>
              ...
              <div style="border-left: 1px solid var(--glass-border); padding-left: 2rem; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center;">
                ${isPaid ? `
                  ...
                  <h3 style="margin-bottom:0.5rem;">Rent Fully Paid</h3>
                  <p style="font-size:0.8rem; color:var(--text-secondary); margin-bottom:1.5rem;">Your reservation is secured! View or download your PDF receipt in the billing tab.</p>
                  <button class="btn btn-secondary" onclick="switchView('payments-view')">View Invoice</button>
                ` : `
                  ...
                  <h3 style="margin-bottom:0.5rem;">Payment Required</h3>
...
async function loadRooms() {
  try {
    ...
    // If guest, fetch recommended stays sorted by match percentage!
    ...
    if (data.success) {
      if (isGuest) {
        ...
        // Extract rooms from recommendation payloads
      ...
      // Populate unique wings in filter dropdown
...
function renderRoomsGrid(rooms) {
  ...
  roomsListGrid.innerHTML = rooms.map(room => {
    ...
    // Render recommendation reasons if present
    const reasonsHtml = room.reasons && room.reasons.length > 0
      ? `<div style="margin-top: 0.5rem; font-size: 0.8rem; color:#c084fc; font-weight: 500;">
    ...
    const matchBadgeHtml = room.matchPercentage !== undefined
      ? `<span class="badge badge-approved" style="background:rgba(139, 92, 246, 0.15); color:#c084fc; border-color:rgba(139, 92, 246, 0.3); font-weight: 700;">${room.matchPercentage}% Match</span>`
      : '';

    return `
      <div class="room-card" style="${room.matchPercentage >= 85 ? 'border-color: rgba(139, 92, 246, 0.3); box-shadow: 0 4px 15px -5px rgba(139, 92, 246, 0.15);' : ''}">
        <div>
...
window.openRequestModal = function(roomId, roomNum, price, type) {
  ...
  // Calculate exact total price including add-ons
  const guests = Number(prefGuests.value) || 2;
  const nights = Number(prefNights.value) || 1;
  ...
  if (prefBedding.checked) {
    total += 30 * nights;
    breakdownStr += `<strong>Extra Bedding:</strong> $30 x ${nights} night(s) = $${(30 * nights).toFixed(2)}<br>`;
  }
  if (prefBreakfast.checked) {
    total += 20 * guests * nights;
    breakdownStr += `<strong>Breakfast Buffet:</strong> $20 x ${guests} guest(s) x ${nights} night(s) = $${(20 * guests * nights).toFixed(2)}<br>`;
  }
  if (prefShuttle.checked) {
    total += 40;
    breakdownStr += `<strong>Airport Shuttle:</strong> $40.00 flat fee<br>`;
  }
  if (prefLateCheckout.checked) {
    total += 30;
...
async function loadPreferencesAndPlanner() {
  ...
  prefBedding.checked = prefs.extraBedding || false;
  
  // Setup add-ons checkboxes
  const addOns = prefs.addOns || [];
  prefBreakfast.checked = addOns.includes('breakfast');
